...
function OrdanetGraph(element, apiUrl, options) {
	this.apiUrl = apiUrl;
    ...
    this.makeTextSprite = function(str, parameters) {
		var sprite = new THREE.SpriteText(str, {
			font: 'Arial',
			bold: true,
			fontColor: parameters && parameters.textColor ? parameters.textColor : 'rgb(255, 255, 255, 1)',
			textHeight: parameters && parameters.textSize ? parameters.textSize : 4,
		...
		fetch(url+(urlParams.length > 0 ? '?'+urlParams.join('&') : ''))
			...
			.then(function(json) {
				...
				json.nodes.forEach(function(node) {
					node.name = node.properties.fletta;
					node.mark = node.properties.mark;

					return node;
				});

				json.links.forEach(function(link) {
					link.sameiginlegir_felagar = ((link.properties.props.sameiginlegir_felagar ? link.properties.props.sameiginlegir_felagar.low : 1)/15)+0.5;
				...
				this.graph.cooldownTicks(300)
					...
					.nodeThreeObject(function(node) {
						...
						return this.makeTextSprite(node.name, textSpriteParams)
					}.bind(this));
	...
	this.startAutoFlight = function(flightSpeed, options) {
		if (!this.flightInterval) {
			...
			}.bind(this), flightSpeed ? flightSpeed+(options.pauseInterval || 5000) : 20000);
	...
	this.highlightNodes = function(nodes) {
		if (this.highlightedNodes && this.highlightedNodes.length > 0) {
			this.highlightedNodes.forEach(function(node) {
				node.__threeObj.material.opacity = 0.8
			});
		}
...
*/
	...
	this.flyToNode = function(node, flightSpeed, distance) {
		...
		var tween = new TWEEN.Tween(position).to(target, flightSpeed || 15000).easing(TWEEN.Easing.Cubic.InOut);
